...
def get_image(image_path,gt_path,image_size,is_crop=True):
    ...
    #input_ = transform(loadmat(image_path),image_size,randx,randy,is_crop)
...
def transform(image, npx,is_crop=True):
    # npx : # of pixels width/height of image
    randx = np.random.randint(image.shape[1]-npx)
    randy = np.random.randint(image.shape[0]-npx)
    if is_crop:
        cropped_image = random_crop(image, npx,randx,randy)
        #cropped_image = center_crop(image, npx)
    else:
